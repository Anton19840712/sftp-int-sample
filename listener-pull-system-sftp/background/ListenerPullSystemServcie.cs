...
{
	public class ListenerPullSystemServcie : IHostedService
	{
		private readonly IConnectionFactory _connectionFactory;
		private readonly ILogger<ListenerPullSystemServcie> _logger;

		// Директория в которую будут сохраняться зачитанные из сетевой шины файлы.
		private readonly string _localDirectory = @"C:\sftp-local-pull-system-listener-storage";
		...
		public ListenerPullSystemServcie(
			SftpConfig config,
			IConnectionFactory connectionFactory,
			ILogger<ListenerPullSystemServcie> logger)
		{
		...
		public async Task StartAsync(CancellationToken cancellationToken)
		{
			...
			_listenerTask = Task.Run(() =>
			{
				...
				_channel.BasicConsume("sftp-pull-system-queue", false, consumer);
			}, cancellationToken);
		}
